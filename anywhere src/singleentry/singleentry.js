...
//constant as per JD Lowe 06/29/2018, DO NOT TOUCH
var isIndiana = true;
...
function buildSingleEntryBanner(addFilterButton) {
    return new Promise(function (fulfill, reject) {
        ...
        var timeEntryButton = $("<button>").addClass("timeentryreportbutton").text("Time Entry Report").click(function () {
            var userId = $.session.UserId;
            if (startDateBox.data("startDate") == undefined || startDateBox.data("endDate") == undefined) { return; }
            var startD = startDateBox.data("startDate");
            var endD = startDateBox.data("endDate");
        ...
        var overlapReportButton = $("<button>").addClass("overlapreportbutton").text("Overlap Report").click(function () {
            var userId = $.session.UserId;
            if (startDateBox.data("startDate") == undefined || startDateBox.data("endDate") == undefined) { return; }
            var startD = startDateBox.data("startDate"),
            endD = startDateBox.data("endDate");
        ...
        $("#actionbanner").append(timeEntryButton).append(overlapReportButton).off("SEReportButtons").on("SEReportButtons", function () {
...
function buildSingleEntryCard(vals, isSaved) {
    return new Promise(function (fulfill, reject) {
        ...
        getRequiredSingleEntryFieldsAjax(function (err, res) {
            ...
            if(vals.hours && !isNaN(parseFloat(vals.hours))) {
                target.text(vals.hours);
            }
            ...
            var saveClick = function (results) {
                ...
                if (isIndiana == false) {
                    ...
                    if ($.session.singleEntryAddConsumersOnBillable == "Y" && results.data.billable == "Y") {
                        if (!$(".singleentryselected").length) {
                            $.fn.PSmodal({
                                body: "You have selected no consumers.  Do you want to save and continue?",
                                immediate: true,
                                buttons: [
                                    {
                                        text: "Yes",
            ...
            var buttonLogic = function () {
                // Handles Total Consumer Count
                var consumerCount = document.getElementsByClassName('consumer singleentryselected');
                var consumerLabel = $('#seConsumersLabel');
                consumerLabel.html("Total Consumers: " + (consumerCount.length));
                ...
                if (singleEntryID != "" && $.session.isPSI != true) {
                ...
                if (readonly) {
                    ...
                    if (!results.errors.length && $.session.isPSI != true) {
            ...
            var consumersLabel = $("<div id='seConsumersLabel'>").html("Total Consumers: ");
...
function finishSingleEntryCard(obj) {
    ...
    window.buildCustomerList = function (myWorkCode) {
        var byLocationOnScreen = function () {
            var ID = locations.attr("locid");
            if ($.session.singleEntryAddConsumersOnBillable == 'P' && !$.session.singleEntryUseServiceLocations) ID = '%';
        ...
        myWorkCode = myWorkCode || "N";
        if (myWorkCode == "N") {
            byLocationOnScreen();
        }
        else {
            /*
            if ($.session.singleEntryAddConsumersOnBillable != 'P') byLocationOnScreen();
            else byLocationMagic();
            */
            byLocationOnScreen();
        }
    }
                ...
                if (!readonly) {
                    target.PSlist(workCodesArr, {
                        callback: function (item) {
                            ...
                            if ($.session.singleEntryUseServiceLocations) {
                                // show locations
                                ...
                                if (isIndiana == true) {
                                    if ($.session.singleEntryAddConsumersOnBillable == "Y" || $.session.singleEntryAddConsumersOnBillable == "N") {
                                        locations.parent().parent().css("display", "inline-block");
                                    }
                                    else {
                                        if (item.billable == "Y") {
...
function parseSingleEntry(res, readonly, isAdmin) {
    ...
    var startTime = $("Start_Time", res).text().split(":"),
    ...
    if ($("singleentry", res).length > 0) {
        ...
        //Hack to get consumers properly shaded by location on edit
        $.session.singleEntryEditLocationHack = vals.locationID;
    ...
    $(".leftmenu-btn").removeClass("buttonhighlight");

    if ($("End_Time", res).text() === "23:59:59") {
        vals.endTime = "12:00 AM";
...
function singleEntryAuditor(opts) {
    ...
    //if ($.session.singleEntryUseServiceLocations && opts.billable == "N") {
    ...
    if (opts.location == undefined && opts.locationBox.is(":visible")) {
        ...
        window.singleentryErrors["locations"].hide();
        result.errors.splice(result.errors.indexOf("Please select a location."), 1);
    }
    ...
    // New as of 7/19/18 - Ash
    if (opts.billable != undefined) {
        if ($.session.singleEntryUseServiceLocations === true && $.session.singleEntryAddConsumersOnBillable == "P") {
            if (opts.billable !== "N") {
                if (!consumerIdString.length) {
                    result.errors.push("Please select at least one consumer.");
                    window.singleentryErrors["consumers"].show();
                }
                if ($.session.singleEntryUseServiceLocations && opts.location == undefined) {
                    result.errors.push("Please select a location.");
                    window.singleentryErrors["locations"].show();
                }
            } else {
                if (opts.location == undefined) {
                    window.singleentryErrors["locations"].show();
                    result.errors.push("Please select a location.");
                    //var a = result.errors.indexOf("Please select a location.");
                    //result.errors.splice(a, 1);
                    //result.errors.splice(result.errors.indexOf("Please select a location."), 1);
                }
            ...
            if (isIndiana == true) {
                if ($.session.singleEntryAddConsumersOnBillable == "P") {
                    if (opts.billable !== "N") {
                        if (!consumerIdString.length && opts.location == undefined) {
                            result.errors.push("Please select at least one consumer.");
                            window.singleentryErrors["consumers"].show();
                        }
                    }
    ...
    if (opts.billable != undefined) {
        if (isIndiana == true) {
            if ($.session.singleEntryAddConsumersOnBillable == "N") {
                if (!consumerIdString.length) {
                    result.errors.push("Please select at least one consumer.");
                    window.singleentryErrors["consumers"].show();
                }
            }
            else if ($.session.singleEntryAddConsumersOnBillable == "Y") {
                if (opts.billable == "N") {
                }
            }
        }
    ...
    */
    ...
    if (opts.keyTimes != undefined) {
        if (opts.keyTimes == "N" && 
            (
                opts.timeDiff == "" ||
                isNaN(parseFloat(opts.timeDiff)) ||
                opts.timeDiff.indexOf("-") != -1 || 
                (
                    $.session.singleEntry15minDoc == "Y" &&
                    parseInt(parseFloat(opts.timeDiff) * 100, 10) % 25 != 0
                )
            )
        ) {
...
function moveAllConsumersToActiveListSE() {
    ...
    debugger;
            if (actioncenterCount.length > 500) {
                ...
                actioncenterCount.each(function () {
                    var obj = $(this);
                    return moveConsumerToActiveList(obj);
                    obj.fadeOut('fast', function () {
                        moveConsumerToActiveList(obj);
                    });
                });
